...
//        dd($validationRules);
        ...
        try {
            ...
            if($error == 0 && $amount > 0){
                ...
                if ($request->hasFile('document')) {
                    $cheque_file_path = $request->file('cheque_file_path')->store('cheque_file_path', 'public');
                }else{
                    $cheque_file_path = '';
                }
                //Payment
    ...
    {
        $candidates = Candidate::with('classGroup')->where('agent_id',CommonClass::user()->profile_id)->orderBy('created_at', 'desc')->get();
...
//        $this->authorize('add', app($dataType->model_name));
        ...
        if($user && !empty($password) && !empty($request->email)){
            ...
            foreach ($owners as $owner){
                ...
                $notification->title = 'New candidate has been added with code '.$data->code;
