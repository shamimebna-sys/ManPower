            ...
            @else
 		    ...
 		    $key = $options->table.'.'.$options->key;
                    ...
                    if (!is_null($dataTypeContent->getKey())) {
                        ...
                        )->pluck( $key)->toArray(); // only get real key
                    }

                    $selected_keys = old($relationshipField, $selected_keys);

                    // Fetch related models with only real columns
                    $relatedModels = app($options->model)->whereIn($key, $selected_keys)->get();
                @endphp
                ...
                >
                    ...
                    @foreach ($relatedModels as $modelItem)
                        <option value="{{ $modelItem->{$options->key} }}" selected="selected">
                            {{ $modelItem->{$options->label} ?? $modelItem->getAttribute($options->label) }}
                        </option>
                    @endforeach
