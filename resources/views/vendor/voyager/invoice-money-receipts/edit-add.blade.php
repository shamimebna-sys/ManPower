...
@section('page_header')
    ...
    <h1 class="page-title" style="float: right">Invoice No.:{{$invoice->invoice_no}} &nbsp; Issue Date:{{$invoice->issue_date}}</h1>
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div style="display: none">
                                    <input type="text"   required class="form-control" name="invoice_id"  value="{{$invoice->id}}">
                                </div>

                                <div class="form-group  col-md-4">
                                    <label for="name">Receipt No <strong style="color: red;">*</strong></label>
                                    @php
                                    $maxReceiptNo = DB::table('invoice_money_receipts')
                                        ->join('invoices', 'invoices.id', '=', 'invoice_money_receipts.invoice_id')
...
//                                        ->where('invoices.agenciers_id', 1)
                                ...
                                <div class="form-group  col-md-4">
                                    ...
                                    @endphp
                                    <input readonly type="text" class="form-control" name="amount" placeholder="amount" value="{{$totalAmount}}">
                                </div>

                                @if(isset($invoice->companier) && isset($invoice->companier->name))
                                <div class="form-group  col-md-4">
                                    <label for="name">Receive From <strong style="color: red;">*</strong></label>
                                    <input readonly type="text" class="form-control" name="received_from" placeholder="received_from" value="{{$invoice->companier->name}}">
                                </div>
                                @else
