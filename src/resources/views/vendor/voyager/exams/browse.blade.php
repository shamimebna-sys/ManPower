                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'relationship')
                                                    @if($row->field == 'exam_belongsto_class_group_relationship')
                                                        @php
                                                            $relationshipData = (isset($data)) ? $data : $dataTypeContent;
                                                            $model = app($row->details->model);
                                                            $selected_ids = array_filter(explode(',', $relationshipData->{$row->details->column}));
                                                            $query = $model::whereIn($row->details->key, $selected_ids)->get();
                                                        @endphp
                                                        @if($query->isNotEmpty())
                                                            @foreach($query as $item)
                                                                <span class="label label-info" style="display: inline-block; margin-bottom: 2px;">{{ $item->{$row->details->label} }}</span>
                                                            @endforeach
                                                        @else
                                                            <span class="text-muted">{{ __('voyager::generic.no_results') }}</span>
                                                        @endif
                                                    @else
