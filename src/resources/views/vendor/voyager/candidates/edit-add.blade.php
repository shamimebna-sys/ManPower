        ...
        <div class="col-md-{{isset($dataTypeContent->id)?'9':'12'}}">
            <div class="page-content container-fluid">
                <form class="form-edit-add" role="form"
                    ...
                    @if(isset($dataTypeContent->id))
                        @if($dataTypeContent->replaced_by_candidate_id)
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="alert alert-warning" style="border-radius: 8px; margin-bottom: 20px; font-size: 14px;">
                                        <i class="voyager-warning"></i> 
                                        <strong>Replacement Notice:</strong> Candidate <strong>{{ $dataTypeContent->code }}</strong> has been replaced by Candidate <strong><a href="{{ route('voyager.candidates.edit', $dataTypeContent->replaced_by_candidate_id) }}?candidate_id={{ $dataTypeContent->replaced_by_candidate_id }}" style="text-decoration: underline; font-weight: bold;">{{ $dataTypeContent->replacedByCandidate->code ?? 'N/A' }} ({{ $dataTypeContent->replacedByCandidate->name ?? 'N/A' }})</a></strong>.
                                    </div>
                                </div>
                            </div>
                        @endif
                        @if($dataTypeContent->replaces_candidate_id)
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="alert alert-info" style="border-radius: 8px; margin-bottom: 20px; font-size: 14px;">
                                        <i class="voyager-info"></i> 
                                        <strong>Replacement Notice:</strong> Candidate <strong>{{ $dataTypeContent->code }}</strong> replaces Candidate <strong><a href="{{ route('voyager.candidates.edit', $dataTypeContent->replaces_candidate_id) }}?candidate_id={{ $dataTypeContent->replaces_candidate_id }}" style="text-decoration: underline; font-weight: bold;">{{ $dataTypeContent->replacesCandidate->code ?? 'N/A' }} ({{ $dataTypeContent->replacesCandidate->name ?? 'N/A' }})</a></strong>.
                                    </div>
                                </div>
                            </div>
                        @endif
                                ...
                                <div class="panel-body">
                                    <div class="row">
                                        ...
                                        <div class="row">
                                         ...
                                         <div class="row">
                                             @include('vendor.voyager.partials.form-field', ['formField'=>'include', 'fields'=>['candidate_belongsto_country_relationship'], 'width'=>'3'])
                                             @include('vendor.voyager.partials.form-field', ['formField'=>'include', 'fields'=>['passport_no'], 'width'=>'3'])
                            ...
                            <div class="panel panel-bordered panel-warning">
                                ...
                                <div class="panel-body">
                                    <div class="row">
                                        ...
                                        <div style="display: {{in_array(\App\Helpers\CommonClass::user()->role_id,[101, 107, 108, 110])?'none':'block'}}">
                                        ...
                                        <div style="display: {{in_array(\App\Helpers\CommonClass::user()->role_id,[101, 107, 108, 109, 110])?'none':'block'}}">
                                        ...
                                        <hr class="col-md-12" style="margin: 15px 0 !important; border-top: 1px solid #f1f5f9;" />
{{--                                         
                                        <div class="section-header col-md-12" style="margin-bottom: 12px; padding: 0 15px;">
                                            <svg class="section-header-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width: 14px; height: 14px; display: inline-block; vertical-align: middle; color: #64748b;">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                                            </svg>
                                            <h4 style="display: inline-block; vertical-align: middle; margin: 0 0 0 6px; font-size: 11px; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: 0.7px;">Candidate Replacement</h4>
                                        </div> --}}

                                        <div class="form-group col-md-12">
                                            <label for="replaced_by_candidate_id">Replaced By</label>
                                            <select class="form-control select2-ajax-candidates" name="replaced_by_candidate_id" id="replaced_by_candidate_id">
                                                @if(isset($dataTypeContent->replacedByCandidate))
                                                    <option value="{{ $dataTypeContent->replaced_by_candidate_id }}" selected>
                                                        ({{ $dataTypeContent->replacedByCandidate->code }}) {{ $dataTypeContent->replacedByCandidate->name }}
                                                    </option>
                                                @else
                    ...
                    <button type="submit" class="btn btn-primary pull-right save" style="display: {{in_array(\App\Helpers\CommonClass::user()->role_id,[ 110])?'none':'block'}}">
...
@section('javascript')
    ...
    <script>
        ...
        function openModalPreview(fileUrl, label, filesList) {
            ...
            if (filesList && filesList.length > 1) {
                filesList.forEach(function(item) {
                    ...
                    $thumb.on('click', function(e) {
                        e.preventDefault();
                        openModalPreview(item.url, label, filesList);
                    });
        ...
        $(document).ready(function() {
            // Candidates replacement select2 initialization
            $('.select2-ajax-candidates').select2({
                ajax: {
                    url: '{{ route("admin.ajax.search-candidates") }}',
                    dataType: 'json',
                    delay: 250,
                    data: function (params) {
                        return {
                            q: params.term,
                            exclude_id: '{{ $dataTypeContent->id ?? "" }}'
                        };
                    },
                    processResults: function (data) {
                        return {
                ...
                placeholder: 'Search & select a candidate',
                allowClear: true,
            ...
            // Handle file attachments (conditionally allowing multiple or last-only)
            var docFields = [
                { name: 'cv_file_path', label: 'CV', lastOnly: true },
                { name: 'nid_file_path', label: 'NID', lastOnly: false },
                { name: 'passport_file_path', label: 'Passport Scan', lastOnly: false },
                { name: 'stamp_file_path', label: 'Stamp Copy', lastOnly: false }
            ];

            docFields.forEach(function(field) {
                var fieldName = field.name;
                var fieldLabel = field.label;
                var lastOnly = field.lastOnly;
                ...
                if ($divs.length > 0) {
                    ...
                    // Build list of files for modal gallery preview
                    var filesList = [];
                    $divs.each(function(index, el) {
                        var $fileLink = $(el).find('.fileType');
                        if ($fileLink.length > 0) {
                            var fileUrl = $fileLink.attr('href');
                            var ext = fileUrl.split('.').pop().toLowerCase().split('?')[0];
                            var name = $fileLink.text().trim();
                            if (!name || name === 'Download') {
                                name = fieldLabel + (lastOnly ? '' : ' (' + (index + 1) + ')');
                            }
                            filesList.push({ url: fileUrl, ext: ext, name: name });
                        }
                    });
                    ...
                    $divs.each(function(index, el) {
                        ...
                        if ($fileLink.length > 0) {
                            ...
                            if (!fileName || fileName === 'Download') {
                                if (!lastOnly && $divs.length > 1) {
                                    fileName = fieldLabel + ' (' + (index + 1) + ').' + fileUrl.split('.').pop().split('?')[0];
                                } else {
                            ...
                            $bar.find('.btn-custom-preview-trigger').on('click', function(e) {
                                e.preventDefault();
                                openModalPreview(fileUrl, fieldLabel, filesList);
                            });
                        }
                    });
                }
