    ...
    @if($data)
        <table style="width: 100%; padding-top: 20px">
            ...
            @if($data)
                ...
                @foreach($data as $s=>$i)
                        @php
                            $latestFlight = $i->flightSchedules->sortByDesc('created_at')->first();
                        @endphp
                        <tr style="border: 1px solid #ddd">
                            ...
                            <td style="padding: 5px;border: 1px solid #ddd;">{{optional($latestFlight)->airlinece_name ?? 'N/A'}}</td>
                            <td style="padding: 5px;border: 1px solid #ddd;">{{optional($latestFlight)->flight_date ?? 'N/A'}}</td>
                            <td style="padding: 5px;border: 1px solid #ddd;">{{optional($latestFlight)->departure_time ?? 'N/A'}}</td>
                            <td style="padding: 5px;border: 1px solid #ddd;">{{optional($latestFlight)->arrival_time ?? 'N/A'}}</td>
                        </tr>
                @endforeach
            @endif
            </tbody>
