...
class VoyagerAgentController extends VoyagerBaseController
{
    private $role_id = 101;
    ...
    public function store(Request $request)
    {
        ...
        $request->merge(['code' => CommonClass::generateCode('A')]);
        $request->merge(['email' => strtolower($request->get('email'))]);
        $request->merge([ 'balance' => $request->filled('balance') ? $request->balance : 0  ]);
        ...
        if($user &&  !empty($request->email)){
            CommonClass::sendEmail('Agent Registration', $request->email, ['username'=>$request->email, 'password'=>$password], 'email.registration');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'email' => 'required|email|unique:users,email,' . User::where('agent_id', $id)->pluck('id')->first(),
        ]);
        if ($validator->fails()) {
            foreach ($validator->messages()->getMessages() as $messages) {
                return redirect()->back()->with($this->alertError(__($messages[0])))->withInput();
