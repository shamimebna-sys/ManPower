...
class VoyagerCompanierController extends VoyagerBaseController
{
    private $role_id = 107;
    ...
    public function store(Request $request)
    {
        ...
        $request->merge(['code' => CommonClass::generateCode('CC')]);
        $request->merge(['email' => strtolower($request->get('email'))]);
        $validator = Validator::make($request->all(), [
            'code' => 'required|unique:companiers,code',
//            'mobile' => 'required|unique:companiers,mobile',
        ...
        $user = User::create([
            ...
            'companier_id'        => $data->id,
            'avatar'        => 'users/default.png'
        ]);
        if($user && !empty($password) && !empty($request->email)){
            CommonClass::sendEmail('Registration', $request->email, ['username'=>$request->email, 'password'=>$password], 'email.registration');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if(!empty($request->password)){
            $userEntity = User::where('companier_id', $data->id)->first();
            $userEntity->password = bcrypt($request->password);
            $user = $userEntity->update();

            if($user){
                CommonClass::sendEmail('Update Information', $request->email, ['username'=>$request->email, 'password'=>$request->password], 'email.registration');
            }
        }
