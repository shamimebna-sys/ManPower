...
class VoyagerEmployeeController extends VoyagerBaseController
{
    private $role_id = 104;
    ...
    public function store(Request $request)
    {
        ...
        $request->merge(['code' => CommonClass::generateCode('E')]);
        ...
        if($user && !empty($password) && !empty($request->email)){
            CommonClass::sendEmail('Employee Registration', $request->email, ['username'=>$request->email, 'password'=>$password], 'email.registration');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if(!empty($request->password)){
            $userEntity = User::where('employee_id', $data->id)->first();
            $userEntity->password = bcrypt($request->password);
            $user = $userEntity->update();

            if($user){
                CommonClass::sendEmail('Update Employee Information', $request->email, ['username'=>$request->email, 'password'=>$request->password], 'email.registration');
            }
        }
