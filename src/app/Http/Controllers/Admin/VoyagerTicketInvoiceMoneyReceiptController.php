...
use App\Models\TicketInvoice as Invoice;
...
class VoyagerTicketInvoiceMoneyReceiptController extends VoyagerBaseController
{
    public function printInvoiceMoneyReceipt(Request $request, $id){
        ...
        $data = InvoiceMoneyReceipt::where('ticket_invoice_id', $id)->first();

        $title = $title.'-'.$data->invoice->invoice_no;
        $downloadable = false;
        $printBlade = 'voyager::reports.print.ticket-invoice-money-receipt';
    ...
    {
        $invoiceId = $request->get('ticket_invoice_id');
        /*if(empty($invoiceId)){
            return redirect()->route("voyager.ticket-invoices.index");
        }*/
...
//        dd($request->all());
        ...
        if (!$request->has('_tagging')) {
            if (auth()->user()->can('browse', $data)) {
                ...
                $redirect = redirect()->route("voyager.ticket-invoices.index");
            } else {
