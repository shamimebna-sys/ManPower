...
class VoyagerEmployerController extends VoyagerBaseController
{
    private $role_id = 105;
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'code' => 'required|unique:employers,code',
        ...
        if($user && !empty($password) && !empty($request->email)){
            CommonClass::sendEmail('Employer Registration', $request->email, ['username'=>$request->email, 'password'=>$password], 'email.registration');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if(!empty($request->password)){
            $userEntity = User::where('employer_id', $data->id)->first();
            $userEntity->password = bcrypt($request->password);
            $user = $userEntity->update();

            if($user){
                CommonClass::sendEmail('Update Employer Information', $request->email, ['username'=>$request->email, 'password'=>$request->password], 'email.registration');
            }
        }
