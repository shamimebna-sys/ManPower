...
//        echo  "<img  src='data:image/png;base64,".$barcode."' /><br>".$code.'<br>';
    ...
    public static function user(){
        ...
        if($user){
            if($user->role->id == 101){
                ...
                $user->profile_id = $user->sub_agent_id;
                $user->profile_slug  = 'sub_agents';
            ...
            }elseif($user->role->id == 107){
                $user->profile_id = $user->companier_id;
                $user->profile_slug  = 'companiers';
            }elseif($user->role->id == 108){
                $user->profile_id = $user->agencier_id;
                $user->profile_slug  = 'agenciers';
            }elseif($user->role->id == 109){
                $user->profile_id = $user->sub_agent_id;
                $user->profile_slug  = 'subagent';
            }else{
            ...
            if($currency){
                if(!empty($amount)){
                    if ($currency->position == 'A'){
                        return $amount.' '. (($symbol)? $currency->symbol:$currency->short_name);
                        return number_format($amount, 2).' '. (($symbol)? $currency->symbol:$currency->short_name);
                    }else{
                        return (($symbol)? $currency->symbol:$currency->short_name). ' '.$amount;
    ...
    public static function generateCode($type = 'C', $id = null, $len = 3){
        ...
        if($type == 'A'){
            ...
            $agent = SubAgent::max('id')+1;
            $code = '2'.str_pad($agent, 3, '0', STR_PAD_LEFT);
        }elseif($type == 'T'){
            $agent = Teacher::max('id')+1;
            $code = '2'.str_pad($agent, 3, '0', STR_PAD_LEFT);
        }elseif($type == 'E'){
            $agent = Employee::max('id')+1;
            $code = '2'.str_pad($agent, 3, '0', STR_PAD_LEFT);
        }elseif($type == 'AA'){
            $agent = Agencier::max('id')+1;
            $code = '2'.str_pad($agent, 3, '0', STR_PAD_LEFT);
        }elseif($type == 'CC'){
            $agent = Companier::max('id')+1;
            $code = '2'.str_pad($agent, 3, '0', STR_PAD_LEFT);
        }elseif($type == 'C'){
            ...
            $code = $code.str_pad($max, 6, '0', STR_PAD_LEFT);
        }else{
    ...
    public static function sendTargetedNotificationAndEmail($targetType, $targetId, $title, $body)
    {
        try {
            $users = [];
            $emails = [];
            ...
            if ($targetType === 'candidate') {
                $user = User::where('candidate_id', $targetId)->first();
                if ($user) {
                    $users[] = $user;
                    if ($user->email) {
            ...
            } elseif ($targetType === 'sub_agent') {
                $user = User::where('sub_agent_id', $targetId)->first();
                if ($user) {
                    $users[] = $user;
                    if ($user->email) {
                        $emails[$user->id] = $user->email;
                ...
                $subAgent = SubAgent::find($targetId);
                if ($subAgent && $subAgent->email) {
                    if ($user) {
                        $emails[$user->id] = $subAgent->email;
                    } else {
                        $dummyUser = new User();
                        $dummyUser->name = $subAgent->name;
                        $dummyUser->email = $subAgent->email;
                        self::sendEmail($title, $subAgent->email, ['user' => $dummyUser, 'title' => $title, 'body' => $body], 'email.generic_notification');
                    }
                }
            } elseif ($targetType === 'agent') {
                $user = User::where('agent_id', $targetId)->first();
                if ($user) {
                    $users[] = $user;
                    if ($user->email) {
                        $emails[$user->id] = $user->email;
                ...
                $agent = Agent::find($targetId);
                if ($agent && $agent->email) {
                    if ($user) {
                        $emails[$user->id] = $agent->email;
                    } else {
                        $dummyUser = new User();
                        $dummyUser->name = $agent->name;
                        $dummyUser->email = $agent->email;
                        self::sendEmail($title, $agent->email, ['user' => $dummyUser, 'title' => $title, 'body' => $body], 'email.generic_notification');
                    }
                }
            } elseif ($targetType === 'owner') {
                $owners = User::where('role_id', 106)->get();
            ...
            foreach ($users as $u) {
                ...
                $email = isset($emails[$u->id]) ? $emails[$u->id] : $u->email;
                if ($email) {
                    self::sendEmail($title, $email, ['user' => $u, 'title' => $title, 'body' => $body], 'email.generic_notification');
                }
            }
        } catch (\Exception $e) {
            \Log::error("sendTargetedNotificationAndEmail error: " . $e->getMessage());
        }
    }
...
//            if($visa && $manpowerFeeCollection && $flight){
    ...
    public static function labourStatus($candidateId = null, $showText=false, $expireCheckColor=false){
        $data = LabourContract::where('candidate_id', $candidateId)->latest()->first();

        if($showText && $data){
            return $data->expire_date;
        }elseif($expireCheckColor && $data){
            $today = date('Y-m-d');
//            return $data->expire_date .' '.$today;
            return (strtotime($data->expire_date) < strtotime($today))?'#ff000040':'';
        }else{
            return ($data);
        }
    ...
    public static function liveStatus($candidateId = null){
        ...
        if($candidateId){
            ...
            if (\App\Helpers\CommonClass::flightStatus($candidate->id)) {
                $stepNo = 10;
                $data = FlightSchedule::where('candidate_id', $candidateId)->latest()->first();
                $extraText = '<small> (Schedule: '.$data->flight_date.' '. $data->flight_time.') </small>';
            }
            ...
            elseif (\App\Helpers\CommonClass::labourStatus($candidate->id)) {
                $stepNo = 6;
                //$data = LabourContract::where('candidate_id', $candidateId)->latest()->first();
               // $extraText = '<small> (Expire At'.$data->expire_date.') </small>';
            }
        ...
        if($liveStatus){
            return $liveStatus->name.$extraText;
        }else{
            return '';
        }
    ...
    public static function arcNo($candidateId = null){
        if(!empty($candidateId)){
            $data = Arc::where('candidate_id', $candidateId)->latest()->first();
            if($data){
                return $data->arc_number;
            }
